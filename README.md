...
At the bottom: **Pubblica** writes `caccia*.json` to GitHub (after `trovaCiclo()` has checked that the `richiede` links contain no cycle `biviImpossibili()` that no node shares a fork with one of its ancestors and `elementiSenzaFoto()` that no node to validate is still a placeholder; each blocks publishing and names the nodes involved), and **Prova su questo telefono** (Try on this phone) switches the player view on this same phone to a separate, yellow-striped test space (`localStorage` keys with the `-prova` suffix) without touching the published hunt or the network.
